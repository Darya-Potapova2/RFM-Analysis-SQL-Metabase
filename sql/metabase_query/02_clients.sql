...
rfm as (
	select f.client_id, f.recency, f.frequency, f.monetary,
		case
			when f.recency > p.recency_perc66 then 3
			when f.recency > p.recency_perc33 then 2
			else 1
		end as recency_gr,
		case
			when f.frequency > p.frequency_perc66 then 1
			when f.frequency > p.frequency_perc33 then 2
			else 3
		end as frequency_gr,
		case
			when f.monetary > p.monetary_perc66 then 1
			when f.monetary > p.monetary_perc33 then 2
			else 3
		end as monetary_gr 	
...
-- клиенты с распредлением по сегментам
clients_segments as (
	select *, 
		case
